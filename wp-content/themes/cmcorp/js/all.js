var all = new function all(){
    ...
    {
        var nome = iframe_maximized.find('#formContainer .form input[type="text"]')[0].value;
        var email = iframe_maximized.find('#formContainer .form input[type="email"]')[0].value;
        $.ajax(
        {
            type: 'POST',
    ...
    var batePapo = function batePapo()
    {
        ...
        if (chat[0] != undefined && iframe_maximized[0] != undefined)
        {
            ...
            var iframe_totally_loaded = setInterval(function()
            {
                if (iframe_maximized.find('#formContainer button')[0] != undefined)
                {
                    iframe_maximized.find('#formContainer button').on('click', function()
                    {
                        get_user_info(iframe_maximized);
                    });
                    clearInterval(iframe_totally_loaded);
                };
            }, 200);
    ...
    var newsletter_form = function newsletter_form()
    {
        
        $( "#wpcf7-f398-o1" ).on( "submit", function( event ) {
          event.preventDefault();
        console.log( $( this ).serialize() );        
        //});     
       //$('form.#wpcf7-f390-o1').html('teste');       
      //  $(".button-cadastro").click(function() {
        var formnewsletter = $("#wpcf7-f390-o1").serialize();  
    ...
    this.init = function init(){
     ...
     //   teste();
        ...
        var bp_timer = setInterval(function()
        {
            ...
            if (bp == true)
            {
                console.log('Chat terminou de carregar');
