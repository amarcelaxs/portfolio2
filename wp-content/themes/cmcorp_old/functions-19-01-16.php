...
function render_header_menu(){
	...
	foreach($novoArrayMenu as $item)
	{
		...
		if (count($item->filhos) > 0) {
			...
			$html.="<li><a class='menu-item ' href='#' data-href='$item->url' >$item->title</a>";
...
function sistema_solucoes($parent_id)
{
	...
	foreach ( $item_solucao as $item ) {
		...
		$html .= '<p>' . $excerpt . '</p>';
		$html .= '<a class="info" style="color:'. $cor .'" href="#modal-sistemas" data-toggle="modal" data-page-id="' . $item->ID . '">Saiba Mais +</a></div></div>';
	}
	$html .= '</div>';
	echo $html;
	...
	foreach ( $item_solucao as $item_modal ){
		...
		$modal .= '<hr>';
		$modal .= '<span><a href="#">Baixar lâmina deste sistema | PDF 3.2Mb</a></span>';
		$modal .= '<hr>';
		$modal .= '<span><a href="#">Baixar apresentação | PPS 2.5Mb</a></span>';
		$modal .= '<hr>';
        if ($link_vimeo != ""){
         $modal .= '<span><a href="'.$link_vimeo.'">Baixar video | MOV 8.7Mb</a></span>';
         $modal .= '<hr class="linhas">';
        }
		$modal .= '<a class="action-btn" href="/contato">AGENDE UMA APRESENTAÇÃO</a>';
